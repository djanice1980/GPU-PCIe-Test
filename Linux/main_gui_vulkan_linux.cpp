// ============================================================================
// GPU-PCIe-Test v3.0 - GUI Edition (Vulkan - Linux)
...
SystemMemoryInfo DetectSystemMemory() {
    ...
    while (std::getline(dmiStream, line)) {
        ...
        if (field == "Type" && slotHasModule) {
            if (value != "Unknown" && !value.empty()) {
                detectedType = value;
                // Check if DDR5 or LPDDR5 for speed interpretation
    ...
    // DDR5/LPDDR5: dmidecode reports speed directly in MT/s
    ...
    if (isDDR5orLater) {
        // DDR5/LPDDR5: speed values are already in MT/s
...
int main(int argc, char* argv[]) {
    ...
    g_app.window = glfwCreateWindow(
        ...
        "GPU-PCIe-Test v3.0 GUI (Vulkan - Linux)", nullptr, nullptr
    );
