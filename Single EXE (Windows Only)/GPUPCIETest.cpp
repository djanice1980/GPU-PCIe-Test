...
struct Buffer {
    ...
    void Create(const VulkanContext& ctx, size_t sz, VkBufferUsageFlags usage, VkMemoryPropertyFlags props) {
        size = sz;
...
TestResults RunCommandLatencyTestVulkan(const VulkanContext& ctx, int iterations, bool continuousMode) {
    const char* name = "Command Latency";
    std::cout << name << "\n";
    PrintDivider();
...
int main() {
    ...
    if (api == GraphicsAPI::D3D12 || api == GraphicsAPI::Both) {
        ...
        results.push_back(RunCommandLatencyTestD3D12(queue.Get(), allocator.Get(), list.Get(), fence.Get(), fenceEvent, fenceValue, 
            config.commandLatencyIters, config.continuousMode));

        // CPU->GPU small transfer latency
        ...
        results.push_back(RunLatencyTestD3D12(("CPU->GPU " + FormatSize(config.smallLatencySize) + " Latency").c_str(),
            device.Get(), queue.Get(), allocator.Get(), list.Get(), fence.Get(), fenceEvent, fenceValue,
            cpuSmall, gpuSmall, config.transferLatencyIters, config.continuousMode));

        // GPU->CPU small transfer latency
        ...
        results.push_back(RunLatencyTestD3D12(("GPU->CPU " + FormatSize(config.smallLatencySize) + " Latency").c_str(),
            device.Get(), queue.Get(), allocator.Get(), list.Get(), fence.Get(), fenceEvent, fenceValue,
    ...
    if (api == GraphicsAPI::Vulkan || api == GraphicsAPI::Both) {
        ...
        // Command latency test
        results.push_back(RunCommandLatencyTestVulkan(ctx, config.commandLatencyIters, config.continuousMode));

        // CPU->GPU small transfer latency
        Buffer smallCpuSrc, smallGpuDst;
        smallCpuSrc.Create(ctx, config.smallLatencySize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        smallGpuDst.Create(ctx, config.smallLatencySize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        results.push_back(RunLatencyTestVulkan(("CPU->GPU " + FormatSize(config.smallLatencySize) + " Latency").c_str(),
        ...
        smallGpuSrc.Create(ctx, config.smallLatencySize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        smallCpuDst.Create(ctx, config.smallLatencySize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        results.push_back(RunLatencyTestVulkan(("GPU->CPU " + FormatSize(config.smallLatencySize) + " Latency").c_str(),
